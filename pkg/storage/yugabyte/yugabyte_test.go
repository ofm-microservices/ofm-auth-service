...
var _ = Describe("RunMigrations", func() {
	It("applies migrations and tolerates no-change reruns", func() {
		...
		var count int
		Expect(dbx.Get(&count, `SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'auth_credentials'`)).To(Succeed())
	...
	It("wraps migration execution failures", func() {
		tempDir, err := os.MkdirTemp("", "auth-service-bad-migrations-*")
		Expect(err).NotTo(HaveOccurred())
		...
		cfg.MigrationsTable = "schema_migrations_auth_service_bad"
	...
	It("resolves relative migration paths", func() {
		wd, err := os.Getwd()
		Expect(err).NotTo(HaveOccurred())
		Expect(os.Chdir(authServiceRoot())).To(Succeed())
		DeferCleanup(func() {
