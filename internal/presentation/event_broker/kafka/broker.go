...
// NewBroker constructs the Kafka broker used by auth-service event adapters.
func NewBroker(cfg config.KafkaConfig) (eventbroker.EventBroker, error) {
	return newBroker(cfg, nil)
}

// NewBrokerWithDB constructs the Kafka broker with durable event claims for
// consumers owned by auth-service.
...
func (b *broker) RunPullConsumer(ctx context.Context, cfg config.PullConsumerConfig, handler eventbroker.MessageHandler) error {
	...
	fmt.Printf("auth kafka consumer starting topic=%s group=%s\\n", cfg.Subject, group)
	...
	for {
		...
		if err != nil {
			fmt.Printf("auth kafka consumer stopped topic=%s group=%s error=%v\\n", cfg.Subject, group, err)
			return err
		}
		attempts := 0
		err = resilience.Retry(ctx, resilience.RetryPolicyFromEnv(), func(attemptCtx context.Context, attempt int) error {
			attempts = attempt
			var claimed idempotency.Event
			if b.db != nil {
				claimed = idempotency.DecodeOrFingerprint(cfg.Subject, msg.Value)
				ok, claimErr := idempotency.ClaimDB(attemptCtx, b.db, claimed)
				if claimErr != nil {
					fmt.Printf("auth kafka durable claim failed topic=%s offset=%d error=%v\\n", cfg.Subject, msg.Offset, claimErr)
					return claimErr
				}
				if !ok {
					fmt.Printf("auth kafka event already claimed topic=%s offset=%d event_id=%s\\n", cfg.Subject, msg.Offset, claimed.EventID)
					return nil
				}
			}
			handlerErr := handler(kafkaprop.Context(attemptCtx, msg.Headers), cfg.Subject, msg.Value)
			if handlerErr != nil {
				fmt.Printf("auth kafka handler failed topic=%s offset=%d error=%v\\n", cfg.Subject, msg.Offset, handlerErr)
			}
			if handlerErr != nil && b.db != nil {
				_ = idempotency.Release(attemptCtx, b.db, claimed.EventID)
			}
			return handlerErr
		})
		if err != nil {
		...
		if err := r.CommitMessages(ctx, msg); err != nil {
			fmt.Printf("auth kafka commit failed topic=%s offset=%d error=%v\\n", cfg.Subject, msg.Offset, err)
			return err
		}
	}
}
